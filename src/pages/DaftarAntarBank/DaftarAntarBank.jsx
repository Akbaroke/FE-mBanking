...
function DaftarAntarBank() {
  ...
  const getListBank = async () => {
    const response = await axios.get(`${process.env.REACT_APP_API_URL}listbank`)
  ...
  const daftarAntarBank = async () => {
    ...
    try {
      await axiosJWT.post(`${process.env.REACT_APP_API_URL}daftar_antarbank`, {
        headers: {
          Authorization: `Bearer ${token}`
        },
        no_rek: noRek,
  ...
  const showInfoRekTujuan = async () => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}ceknomorlain`, {
        userId: userId,
        no_rek: noRek,
        bank: bank
      })
