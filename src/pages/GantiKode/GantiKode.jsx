...
function GantiKode() {
  ...
  const getUserNameByEmail = async () => {
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}getInfoUsernameByEmail`, {
        email: email
      })
  ...
  const sendReqOtp = async (nama) => {
    try {
      // send otp
      await axios.post(`${process.env.REACT_APP_API_URL}otp`, {
        nama: nama,
        emailTo: email,
        ip_address: ip
      })
  ...
  const cekLimitResendCode = async () => {
    const response = await axios.post(`${process.env.REACT_APP_API_URL}otp/ceklimit`, {
      ip_address: ip
    })
  ...
  const OtpAuth = async () => {
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}otp/auth`, {
        emailTo: email,
        otpKode: otp,
        ip_address: ip
      })
  ...
  const requestGantiKode = async () => {
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}gantikode`, {
