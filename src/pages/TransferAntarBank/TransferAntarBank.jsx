...
function TransferAntarBank() {
  ...
  const getListBankTerdaftar = async () => {
    const response = await axiosJWT.post(`${process.env.REACT_APP_API_URL}listBankTerdaftar`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    })
  ...
  const getListnorekTerdaftar = async () => {
    const response = await axiosJWT.post(`${process.env.REACT_APP_API_URL}listRekening`, {
      userId: user.userId,
      bank: bank
    })
    console.log(bank);
